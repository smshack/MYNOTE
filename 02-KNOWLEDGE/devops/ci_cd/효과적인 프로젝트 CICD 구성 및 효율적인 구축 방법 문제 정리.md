...
flowchart TD
  %% ───────────────────────────────
  %% 개발자 → GitLab → Jenkins 트리거
  %% ───────────────────────────────
  A["👨‍💻 개발자"] --> B["코드 Push 또는 Merge Request"]
  B --> C["GitLab 저장소"]
  C --> D["Webhook으로 Jenkins 파이프라인 트리거"]

  %% ───────────────────────────────
  %% Jenkins 단계별 CI 수행
  %% ───────────────────────────────
  D --> E["Jenkins: 테스트 단계 (Test)"]
  E --> F["Jenkins: 코드 스타일 검사 (Lint)"]
  F --> G["Jenkins: 애플리케이션 빌드 (Build)"]
  G --> H["SonarQube: 정적 분석 수행"]
  H --> I["품질 게이트 결과 확인"]

  %% ───────────────────────────────
  %% 품질 통과 / 실패 분기
  %% ───────────────────────────────
  I -->|✅ 통과| J["Docker 이미지 빌드"]
  I -->|❌ 실패| FailNotify["Mattermost: 빌드 실패 알림"]
  FailNotify --> C

  %% ───────────────────────────────
  %% Harbor 이미지 관리 및 보안 스캔
  %% ───────────────────────────────
  J --> K["Harbor: 이미지 푸시"]
  K --> L["Harbor: 취약점 스캔"]
  L -->|🟢 취약점 없음| M["배포 단계로 이동"]
  L -->|🔴 취약점 발견| VulnNotify["Mattermost: 취약점 알림 및 배포 중단"]

  %% ───────────────────────────────
  %% 배포 환경 분기
  %% ───────────────────────────────
  M --> N["배포 환경 선택 (Dev / Stage / Prod)"]
  ...
  %% Mattermost 알림
  %% ───────────────────────────────
  O1 --> NotifyDev["Mattermost: Dev 배포 결과 알림"]
  O2 --> NotifyStage["Mattermost: Stage 배포 결과 알림"]
  O3 --> NotifyProd["Mattermost: Prod 배포 결과 알림"]

  %% ───────────────────────────────
  %% 시스템 구성 설명
  %% ───────────────────────────────
  subgraph 시스템 구성요소
    X1["GitLab : 코드 저장소 및 파이프라인 트리거"]
    X2["Jenkins : CI/CD 자동화 서버"]
    X3["SonarQube : 정적 분석 및 품질 게이트"]
    X4["Harbor : 이미지 저장소 및 보안 스캔"]
    X5["Mattermost : 빌드 및 배포 알림"]
  end
...
## kubernetes 환경에서의 배포(argoCD + helm 추가)
```mermaid
flowchart TD
  %% ───────────────────────────────
  %% 개발자 → GitLab → Jenkins 트리거
  ...
  J --> K["Harbor: 이미지 푸시"]
  K --> L["Harbor: 취약점 스캔"]
  L -->|취약점 없음| M["Helm 차트 업데이트"]
  L -->|취약점 발견| VulnNotify["Mattermost: 취약점 알림 및 배포 중단"]

  %% ───────────────────────────────
  %% GitLab → ArgoCD 자동 배포
  %% ───────────────────────────────
  M --> N["GitLab: Helm 차트 저장소 반영"]
  N --> O["ArgoCD: GitOps 동기화 감지"]
  O --> P["쿠버네티스 클러스터 배포 (Dev / Stage / Prod)"]
  ...
  %% Mattermost 알림
  %% ───────────────────────────────
  Q1 --> NotifyDev["Mattermost: Dev 배포 결과 알림"]
  Q2 --> NotifyStage["Mattermost: Stage 배포 결과 알림"]
  Q3 --> NotifyProd["Mattermost: Prod 배포 결과 알림"]

  %% ───────────────────────────────
  %% 시스템 구성 설명
  %% ───────────────────────────────
  subgraph 시스템 구성요소
    X1["GitLab : 코드 저장소 및 Helm 차트 관리"]
    X2["Jenkins : CI/CD 자동화 서버"]
    X3["SonarQube : 정적 분석 및 품질 게이트"]
    X4["Harbor : 이미지 저장소 및 보안 스캔"]
    X5["ArgoCD : GitOps 기반 CD (자동 배포)"]
    X6["Mattermost : 빌드/배포 알림"]
  end
